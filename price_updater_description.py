...
def create_output_df(merged_df, itx_price_c, vendor_price_c):
	"""Calculates the values for the output file."""
	merged_df[PERCENT] = (merged_df[vendor_price_c] - merged_df[itx_price_c]) / merged_df[itx_price_c]
...
def main(
    ...
    output_df = create_output_df(merged_df, itx_price_c, vendor_price_c)

    # update the prices
    itx_df[itx_price_c] = np.where(itx_df[itx_sku_c].isin(vendor_df[vendor_sku_c]), vendor_df[vendor_price_c], itx_df[itx_price_c])


	now = datetime.now()
	current_time = now.strftime("%m-%d-%Y_%H:%M:%S")

    itx_df.to_excel(f"{intertex_file}_{current_time}.xlsx")
    output_df.rename(columns={itx_price_c: OLD_PRICE, vendor_price_c: NEW_PRICE})
    output_df.to_excel(f"{flagged}_{current_time}.xlsx")
